...
WHERE EXISTS (
    SELECT 1
    FROM FacultyAssignments F2
    WHERE F1.faculty_id != F2.faculty_id
      AND F1.assignment_id != F2.assignment_id
      AND F1.start_time = F2.start_time 
      AND F1.end_time = F2.end_time 
      AND F1.department_id != F2.department_id
);
--5n
SELECT
    F1.faculty_id,
    F1.department_id AS Department1,
    F2.department_id AS Department2,
    F1.start_time,
    F1.end_time,
    F2.start_time AS Start_Time2,
    F2.end_time AS End_Time2,
    F1.assignment_id AS AssignmentID1,
    F2.assignment_id AS AssignmentID2
FROM 
    FacultyAssignments F1
JOIN 
    FacultyAssignments F2 ON F1.faculty_id = F2.faculty_id AND F1.assignment_id != F2.assignment_id
WHERE 
    ((F1.start_time <= F2.start_time AND F1.end_time >= F2.start_time) OR
    (F1.start_time <= F2.end_time AND F1.end_time >= F2.end_time)) 
    AND F1.department_id != F2.department_id;
...
SELECT 
    e.student_id,
    COUNT(DISTINCT ce.course_id) AS distinct_courses
FROM 
    Enrollment e
JOIN 
    CourseEnrollments ce ON e.enrollment_id = ce.enrollment_id
